...
def RNN(X, weights, biases, n_steps, n_hidden_units, batch_size, n_dim):
...
if __name__ == '__main__':
    ...
    pred = RNN(x, weights, biases, n_steps, n_hidden_units, batch_size, n_dim)
    ...
    label_v = [0.0, 1.0]
    listNum = len(tr_data)
    tr_data_case, tr_y_case = data.generateRepeatList(listNum/2, case_id - n_steps, case_id, label_v)
    tr_data_anomaly, tr_y_anomaly = data.generateRepeatList(listNum/2, case_id, case_id + n_steps, label_v)
    batch_xs = merge_list(tr_data, tr_data_case)
    batch_ys = merge_list(tr_y, tr_y_case)


    batch_x2 = merge_list(tr_data, tr_data_case)

    batch_xs = merge_list(batch_xs, tr_data_anomaly)
    batch_ys = merge_list(batch_ys, tr_y_anomaly)
    batch_x2 = merge_list(batch_x2, tr_y_anomaly)
