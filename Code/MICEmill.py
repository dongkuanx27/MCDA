...
def RNN(X, weights, biases, instance_length, stepwidth, n_hidden_units, batch_size, n_dim, num_stacked_layers, in_keep_prob, out_keep_prob, w, V, U):
    ...
    results = tf.matmul(res, weights['out']) + biases['out']
    return results
...
if __name__ == '__main__':
    ...
    pred = RNN(x, weights, biases, instance_length, instance_step_width, n_hidden_units, batch_size, n_dim, num_stacked_layers, in_keep_prob, out_keep_prob, w, V, U)
    ...
    with tf.Session() as sess:
        ...
        while epoch < training_iters:
            ...
            for i in range(0, len(training_bt)):
                ...
                _, ac = sess.run([train_op, accuracy], feed_dict={x: bt, y: by})
        ...
        for i in range(0, len(testing_bt)):
            ...
            ac = sess.run(accuracy, feed_dict={x: bt, y: by})
            test_accuracy += ac
        test_accuracy /= len(testing_bt)
        print("testing ac: ", test_accuracy)